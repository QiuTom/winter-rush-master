...
var WRSnow = function() {
	...
	function init(){
		...
		//var barGeom = new THREE.TetrahedronGeometry(250, 2);
		var barGeom = new THREE.SphereGeometry(20);
		...
		skyMaterial = new THREE.MeshBasicMaterial( { map: THREE.ImageUtils.loadTexture('res/img/liming.jpg')} );
		
		var planeGeometry = new THREE.PlaneGeometry( 1600, 600,1,1 );
	...
	function shift(){
		...
		for (i = 0; i < BAR_COUNT; i++) {
			...
			if (p.z + WRGame.getMoverGroup().position.
			z > WRConfig.FLOOR_DEPTH/2){
